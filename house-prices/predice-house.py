...
pretty_metrics = {'neg_mean_squared_error': 'MSE',
...
def clean_datasets(train, submission):
    ...
    for col in object_columns:
        ...
        submission = train.merge(dum, left_index=True, right_index=True)
...
model = xgb.XGBRegressor(max_depth=5, min_child_weight=0.7,
                         subsample=1, eta=0.15,
                         num_round=100, seed=model_seed)
...
for _ in range(1,11):
    ...
    score = model_selection.cross_val_score(model, X_test, y_test,
                                            cv=kfold, scoring=metrics[chosen])
