...
def investigate(train, month):
    ...
    unique_items_month = train.where(
        train['date_block_num'] > month).item_id.unique()
    ...
    print("{} %".format(
        round(len(train_unique.index) / len(train.index) * 100), 2))
...
train_items = train.groupby(['item_id', 'date_block_num'],
                            as_index=False).agg({
                                'item_cnt_day': sum
                            })
...
train = pd.merge(
    ...
    item_categories,
    how="left",
    on=['item_category_id', 'item_category_id'])
...
if train_model:
    ...
    X = train[['shop_id', 'item_id', 'item_category_id',
...
print(test[['shop_id', 'item_id', 'item_category_id',
            'date_block_num']].values)
predict = model.predict(
    test[['shop_id', 'item_id', 'item_category_id', 'date_block_num']].values)
"""
predict = []
...
test = test.drop(
    ['item_category_name', 'item_name', 'item_category_id', 'date_block_num'],
    axis=1)
